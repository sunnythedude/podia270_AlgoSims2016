...
void FlowField::setPerlin2() {
	for (int y = 0; y<flowList.size(); y++) {
		for (int x = 0; x<flowList[y].size(); x++) {
			float noise = ofMap(ofNoise(x*0.05, y*0.05, ofGetElapsedTimef() * 0.5), 0, 1, 0, TWO_PI);
			flowList[y][x].set(ofVec2f(sin(noise) * 100.0, cos(noise) * 100.0));
		}
	}
}

void FlowField::setPerlin3() {
	for (int y = 0; y<flowList.size(); y++) {
		for (int x = 0; x<flowList[y].size(); x++) {
			float noise = ofMap(ofNoise(x, y, ofGetElapsedTimef() * 0.5), 0, 1, 0, TWO_PI);
			flowList[y][x].set(ofVec2f(sin(ofRandom(30, 180)) * 30.0, cos(ofRandom(30, 180)) * 30.0));
		}
	}
}
